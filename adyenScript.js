// ==== SECCIÓN DE UTILIDADES (Generación de Datos Aleatorios) ====
...
// ==== FIN SECCIÓN DE UTILIDADES ====


// ==== EXPOSICIÓN GLOBAL PARA CONSOLA / USO EXTERNO ====
if (typeof window !== 'undefined') {
    window.encryptAdyenData = encryptAdyenData;
    window.sendGenericAdyenRequest = sendGenericAdyenRequest;

    // Opcionalmente, exponer otras funciones si se considera útil para depuración o uso directo:
    // window.getNames = getNames;
    // window.getAddress = getAddress;
    // window.getRandomInt = getRandomInt;

    console.log("Adyen Script Helper cargado. Funciones disponibles en window: encryptAdyenData, sendGenericAdyenRequest.");
    console.log("Ejemplo de uso en consola:");
    console.log(`
    // 1. Definir los datos de la tarjeta
    const cardData = { cc: "4111111111111111", mm: "12", yy: "2030", ccv: "123" };

    // 2. Definir la clave pública de Adyen (reemplazar con la real)
    const adyenKey = "10001|..."; // Clave pública completa

    // 3. Definir el endpoint
    const targetUrl = "https://tu-endpoint.com/api/payments";

    // 4. Definir cómo generar el payload
    const myPayloadGenerator = (encryptedData, additionalInfo) => {
        const params = new URLSearchParams();
        params.append("adyenEncryptedCardNumber", encryptedData.encryptedCardNumber);
        params.append("adyenEncryptedExpiryMonth", encryptedData.encryptedExpiryMonth);
        params.append("adyenEncryptedExpiryYear", encryptedData.encryptedExpiryYear);
        params.append("adyenEncryptedSecurityCode", encryptedData.encryptedSecurityCode);
        // Añadir otros campos necesarios desde additionalInfo o fijos
        // params.append("orderId", additionalInfo.orderId);
        // params.append("shopperReference", "user123");
        return params;
    };

    // 5. (Opcional) Configuración adicional del request
    const config = {
        method: 'POST',
        headers: { 'X-Custom-Header': 'valor_custom' },
        additionalData: { orderId: "ORDER-12345" } // Datos para pasar al payloadGenerator
    };

    // 6. Enviar el request
    sendGenericAdyenRequest(cardData, adyenKey, targetUrl, myPayloadGenerator, config)
        .then(response => response.text()) // o response.json()
        .then(data => console.log("Respuesta del servidor:", data))
        .catch(error => console.error("Error:", error));
    `);
}
// ==== FIN EXPOSICIÓN GLOBAL ====


// ==== Biblioteca Adyen CSE (Client-Side Encryption) ====
// Esta es la biblioteca principal de Adyen para la encriptación en el lado del cliente.
(function() {
