...
async function handleFallbackPayment() {
    ...
    function attemptProceedWithEncryption() {
        if (window.adyen && window.adyen.key && window.adyen.generationTime) {
            console.log("handleFallbackPayment (re-apply): Adyen API is ready. Proceeding with payment.");
    ...
    function handleAdyenNotReady() {
        console.log("handleFallbackPayment (re-apply): Adyen API not ready yet. Setting up listeners...");
        responseArea.innerHTML = 'Esperando inicialización de Adyen...';

        const adyenReadyListener = function(event) {
            console.log("handleFallbackPayment (re-apply): adyenApiReady event received.", event.detail);
            cleanupListeners();
            proceedWithEncryptionAndPayment();
        };

        const adyenFailedListener = function(event) {
            console.error("handleFallbackPayment (re-apply): adyenApiFailed event received.", event.detail);
            cleanupListeners();
            responseArea.innerHTML = `Error: Falló la inicialización de Adyen API. ${event.detail.message || ''}`;
        };

        const timeoutId = setTimeout(() => {
            console.log("handleFallbackPayment (re-apply): Timeout reached.");
            cleanupListeners(); // Limpiar listeners para evitar ejecuciones múltiples
            // Volver a verificar el estado de Adyen una última vez después del timeout
            if (window.adyen && window.adyen.key && window.adyen.generationTime) {
                console.log("handleFallbackPayment (re-apply): Adyen API became ready after timeout check. Proceeding.");
                proceedWithEncryptionAndPayment();
            } else if (responseArea.innerHTML === 'Esperando inicialización de Adyen...') {
                // Solo mostrar error de timeout si no se resolvió por un evento 'failed'
                console.error("handleFallbackPayment (re-apply): Timeout. Adyen API did not become ready.");
                responseArea.innerHTML = 'Error: Timeout esperando la API de Adyen.';
            }
        }, 5000); // Aumentado a 5 segundos para dar más margen

        function cleanupListeners() {
            document.removeEventListener('adyenApiReady', adyenReadyListener);
            document.removeEventListener('adyenApiFailed', adyenFailedListener);
            clearTimeout(timeoutId);
        }

        document.addEventListener('adyenApiReady', adyenReadyListener, { once: true });
        document.addEventListener('adyenApiFailed', adyenFailedListener, { once: true });
    }

    attemptProceedWithEncryption(); // Intentar proceder, o configurar listeners si no está listo.
}

async function proceedWithEncryptionAndPayment() {
    const responseArea = document.getElementById('fallback-response-area');
    responseArea.innerHTML = 'Procesando (Adyen API lista)...';
